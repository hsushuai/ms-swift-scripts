DS_CONFIG=/data01/xushuai/code/ds_config/zero3.json
DATA_VERSION=8
...
HF_LLAMA_PATH=/data01/LLM_model/Qwen1.5-MoE-A2.7B
...
MEGA_DS_LLAMA_PATH="/data01/LLM_model/Qwen1.5-7b-mega-ds-T${TP}P${PP}"
...
if [ "$1" = "convert_hf2mds" ]; then
    DS_CONFIG_PATH="/data01/xushuai/code/Megatron-DeepSpeed/examples_deepspeed/finetune_hf_llama/ds_config_empty.json"
elif [ "$1" = "convert_mds2hf" ]; then
    DS_CONFIG_PATH="/data01/xushuai/code/Megatron-DeepSpeed/examples_deepspeed/finetune_hf_llama/ds_config_empty.json"
else
    DS_CONFIG_PATH="/data01/xushuai/code/Megatron-DeepSpeed/examples_deepspeed/finetune_hf_llama/ds_config.json"
fi

covert_hf2mds_args="deepspeed Megatron-DeepSpeed/tools/hf2megads_weight_converter.py \
--hf-ckpt-num-shards 2 \
--hf-ckpt-dir $HF_LLAMA_PATH \
--load-mode auto \
--save $MEGA_DS_LLAMA_PATH"

covert_mds2hf_args="deepspeed Megatron-DeepSpeed/tools/hf2megads_weight_converter.py \
--hf-ckpt-num-shards 2 \
--hf-ckpt-dir $HF_LLAMA_PATH \
--load-mode auto \
--to-hf-ckpt \
...
finetune_args="deepspeed Megatron-DeepSpeed/finetune_llama.py \
--load $MEGA_DS_LLAMA_PATH"
